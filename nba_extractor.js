...
function parseOtherEvents(raw_txt, metrics) {
    if (raw_txt.search(/Jump Ball (.*) vs. (.*)/) != -1) {
        ...
        try {
            var array = raw_txt.match(/^(\S+) (?:(.*) Foul|(.*)\.Foul)/);
            metrics.eventtype = 'foul';
            metrics.player = array[1];
        ...
        try {
            var array = raw_txt.match(/^(.*) (?:(Bad Pass|(?:Out of Bounds|Poss )?Lost Ball|Traveling|Foul|(?:3 Second |Jump Ball )Violation|Offensive Goaltending|Kicked Ball Violation|Step Out of Bounds|Palming) Turnover|Turnover: (.*) \()/);
...
function readdir(err, files) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        var errFile = [];
        (function(file) {
            var extracted_data = fs.readFile(file, function(err, data) {
                currentFile = file;
                if (err) {
                    errFile.push(file);
                    console.log(err);
                ...
                for (var j = 1; j < lines.length; j++) {
                    ...
                    var extracted_columns = new Array(35);
                    for (var i = 0; i < 10; i++) {
                        extracted_columns[i] = "";
                    }
                ...
                //    }
                // }
                fs.writeFile(extracted_data_dir + '/' + file.replace(/.*\w\/(.*)-raw.csv/, "$1-ex.csv"), extracted_data, function(err) {
                    if (err) {
                        console.log(err);
                    } else {
